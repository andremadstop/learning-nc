...
- **Plan 06 is NOT complete** — it ends with a `checkpoint:human-verify` (blocking). All `type=auto` tasks (1+2) are done, committed, and the frontend is deployed; Task 3 (the live walkthrough) is PAUSED awaiting Andre's approval. `155-06-SUMMARY.md` is deliberately NOT written until then (task override). Commits: `316bd1b` (Certificate.vue + vendored QR + helper + i18n), `d4ff9e3` (tests + CourseSummary entry + i18n).
