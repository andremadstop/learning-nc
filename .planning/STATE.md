...
current_plan: 05
status: in-progress
stopped_at: "155-07 LIVE PROVISIONING DONE (multi-AI review passed SHIP — eb4de42); STOPPED before the browser-only 155-06 human-verify (Andre's step). LIVE MUTATIONS performed on devcloud (relais): (1) info.xml 4.4.7→4.4.8 (commit 77e1159, migration-trigger bump only — NOT v5.0.0); (2) deploy-prod.sh --php-only (PHPStan L5 clean); (3) occ upgrade → applied Version009100 (additive CREATE TABLE oc_learning_cert_keys + oc_learning_certificates on live PG16; maintenance mode auto on→off; site 302 OK after; occ upgrade also pulled a pending spreed/Talk app update as side-effect — non-destructive); (4) occ learning:cert:init-issuer → ONE Ed25519 issuer key, key_id=UI3V-D_j57IeIOlPBAW-2VQRu0dHB2lkZ0rDLj-LBU4. REAL GREEN live: schema (both tables + active_idem_key UNIQUE index learn_cert_idem_uq confirmed by name via psql), CERT-01 (key generated; re-run refuses w/ exit 1), CERT-02 (did.json HTTP 200, publicKeyJwk only, affirmative NO-secret [no 'd'/secret/secret_key_enc], x==kid fragment, host did:web:devcloud.andrestiebitz.de), CERT-03 (LeakageAuditTest 3/3=39 assertions live + grep gate). did.json id-format assertion GREEN unauthenticated. DEFERRED-TO-HUMAN (ride on Andre's 155-06 human-verify pass): Gate 2 test-api.sh authenticated suite (no admin creds — vault DevCloud-Zugangsdaten.md is a dangling wikilink, absent on workstation+cockpit; chromium login mechanism probed OK so runs green once Andre supplies ADMIN_PASS) + the real-cert independent verify (verify-issued-cert-gate.sh needs a real issued JWT; verifier MECHANISM already GREEN on synthetic JWT; python3-cryptography 43.0.0 present in-container) + the rotation-preserves live check. Requirements NOT marked complete (checkpoint — Andre's human-verify closes CERT-05/07/08/09/10/12/13 + the real-cert kid assertion). NO SUMMARY written (plans 06+07 paused, not closed). Minor noted follow-up: InitIssuerCommand CLI prints did:web:localhost (CLI getBaseUrl host-resolution) while served did.json + HTTP-context issuance correctly use devcloud.andrestiebitz.de — cosmetic only, nothing host-specific is persisted."
last_updated: "2026-06-27T12:00:00.000Z"
last_activity: 2026-06-27 — 155-07 LIVE PROVISIONING executed on devcloud (review passed SHIP). info.xml 4.4.8 (77e1159) + occ upgrade applied Version009100 (both cert tables + active_idem_key UNIQUE on live PG16) + occ learning:cert:init-issuer (key_id UI3V-D_j…). GREEN live: schema, CERT-01 (key+refuse), CERT-02 (did.json 200, no-secret, x==fragment), CERT-03 (LeakageAuditTest 39 assertions). Site up, maintenance off. DEFERRED to Andre's 155-06 human-verify: Gate 2 (no admin creds) + real-cert independent verify + rotation check. Requirements left Pending; no SUMMARY (plans paused).
progress:
  total_phases: 4
  completed_phases: 1
  total_plans: 7
...
Plan: 155-05 complete (5/7); 155-07 LIVE PROVISIONING done; 155-06 + 155-07 both PAUSED at Andre's browser human-verify
Status: The multi-AI review PASSED SHIP (eb4de42), so 155-07 LIVE PROVISIONING was executed on devcloud (relais). Live mutations: info.xml 4.4.7→4.4.8 (commit 77e1159, migration-trigger only), deploy --php-only (PHPStan L5 clean), `occ upgrade` applied the additive Version009100 (CREATE TABLE oc_learning_cert_keys + oc_learning_certificates on live PG16 — maintenance mode auto on→off, site 302 OK after; occ upgrade also pulled a pending spreed/Talk update as a non-destructive side-effect), and `occ learning:cert:init-issuer` generated ONE Ed25519 issuer key (key_id UI3V-D_j57IeIOlPBAW-2VQRu0dHB2lkZ0rDLj-LBU4). REAL GREEN live: schema (both tables + the active_idem_key UNIQUE index learn_cert_idem_uq confirmed by name via psql; cert_keys.secret_key_enc is TEXT), CERT-01 (key generated + re-run refuses, exit 1, one active key), CERT-02 (did.json HTTP 200, publicKeyJwk only, affirmative no-secret check [no 'd'/secret/secret_key_enc], x==kid fragment, host did:web:devcloud.andrestiebitz.de), CERT-03 (LeakageAuditTest 3/3 = 39 assertions live + grep gate clean). DEFERRED-TO-HUMAN (close on Andre's 155-06 pass): Gate 2 test-api.sh authenticated suite (no admin creds available — vault DevCloud-Zugangsdaten.md is a dangling wikilink, absent on workstation+cockpit; chromium login mechanism probed OK, so it runs green the moment Andre supplies ADMIN_PASS), the real-cert independent verify (verify-issued-cert-gate.sh — verifier mechanism already GREEN on a synthetic JWT, python3-cryptography 43.0.0 present in-container, needs a real issued JWT), and the rotation-preserves live check. Requirements left Pending (checkpoint, not plan-close). No SUMMARY written for 06/07 (both paused).
Last activity: 2026-06-27 — 155-07 LIVE PROVISIONING run (info.xml 4.4.8 commit 77e1159 + occ upgrade Version009100 + init-issuer). See Execution Notes (155-07 LIVE).

Progress: [███████░░░] 71% (5/7 plans complete; live issuer provisioned; 155-06 + 155-07 both paused at Andre's browser human-verify)
...
- **⚠ OPEN FINDING (needs follow-up before relying on CERT-12):** the `certificate_issued` notification persisted (object_id+timestamp match the cert), BUT its recipient is recorded as `user=oc_admin` (the Postgres role — not even an existing NC user) instead of the student `zz-test-cert155`. (Triggers 2–3 short-circuit on the `issueIfPassed` idempotency guard BEFORE `notify()` runs, so notify executed exactly once; the notification getCount-dedup path was NOT exercised. `certificate_issued` is emitted ONLY by IssuanceService.php:233 — confirmed.) Deployed `IssuanceService::notify()` verifiably calls `setUser($userId)` with the correct student id (cert.user_id and the course.passed audit event are both zz-test-cert155), so the misattribution originates BELOW the app layer (NC notification storage / actor resolution under app-password auth). Student would NOT see their own cert notification.
...
Last session: 2026-06-27T12:00:00.000Z
Stopped at: 155-07 LIVE PROVISIONING done (review passed SHIP). Live on devcloud: info.xml 4.4.8 (77e1159) + occ upgrade → Version009100 (both cert tables + active_idem_key UNIQUE on PG16) + occ learning:cert:init-issuer (key_id UI3V-D_j…). GREEN live: schema, CERT-01 (key+refuse), CERT-02 (did.json 200 / no-secret / x==fragment), CERT-03 (LeakageAuditTest 39 assertions). Site up, maintenance off. DEFERRED to Andre's 155-06 human-verify: Gate 2 test-api.sh (no admin creds — chromium mechanism OK, needs ADMIN_PASS), real-cert independent verify (mechanism GREEN, needs a real issued cert), rotation-preserves (destructive). Requirements left Pending; 155-06-SUMMARY + 155-07-SUMMARY deliberately NOT written (both plans paused, not closed — human-verify is the close).

Resume: Andre's browser human-verify (155-06 Task 3 + 155-07 steps 9-13 of the walkthrough below). Then mark CERT-01..13, write 155-06/155-07 SUMMARYs, /gsd:verify-work → Phase 156/157.
Resume file: .planning/phases/155-certificate-artifact-issuer/155-06-PLAN.md (Task 3 human-verify) + 155-07-PLAN.md
