...
class Version000350Date20260213000000 extends SimpleMigrationStep {
    public function changeSchema(IOutput $output, \Closure $schemaClosure, array $options): ?ISchemaWrapper {
        ...
        if (!$schema->hasTable('learning_question_translations')) {
            $table = $schema->createTable('learning_question_translations');
            $table->addColumn('id', Types::BIGINT, ['autoincrement' => true, 'notnull' => true, 'unsigned' => true]);
        ...
        // Answer translations
        if (!$schema->hasTable('learning_answer_translations')) {
            $table = $schema->createTable('learning_answer_translations');
            $table->addColumn('id', Types::BIGINT, ['autoincrement' => true, 'notnull' => true, 'unsigned' => true]);
