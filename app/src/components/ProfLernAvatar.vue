<template>
	<div class="proflern-avatar-wrapper">
		<div
			class="proflern-avatar"
			:class="[`animation-${normalizedAnimation}`, { 'has-message': hasMessage, 'is-waving': normalizedAnimation === 'wave' }]">
...
const SUPPORTED_ANIMATIONS = ['idle', 'talk', 'wave', 'celebrate']
...
export default defineComponent({
	...
	setup(props) {
		const normalizedAnimation = computed(() => {
			if (SUPPORTED_ANIMATIONS.includes(props.animation)) {
				return props.animation
			}
			if (props.animation === 'thinking') {
				return 'idle'
			}
			return 'talk'
		})
...
.proflern-avatar.has-message {
	transform-origin: bottom center;
}

.proflern-avatar-wrapper:hover .proflern-avatar {
	transform: scale(1.08) translateY(-3px);
	filter: drop-shadow(0 6px 16px rgba(44, 108, 159, 0.45));
}

.proflern-avatar-wrapper:hover .pupils,
.proflern-avatar-wrapper:hover .glasses {
	transform: translateY(-1.5px);
	transition: transform 0.2s ease-out;
}

.proflern-avatar-wrapper:hover .eyebrows {
